            ...
            <el-table-column label="操作" width="228" class-name="record-actions-column">
              <template #default="scope">
                <div class="record-actions">
                  <el-tooltip
                    v-if="canViewConfig(section.name, scope.row)"
                    :content="configButtonTip(section.name)"
                    placement="top"
                  >
                    <el-button
                      size="small"
                  ...
                  <el-tooltip
                    v-if="section.name === 'dashboardConfigs'"
                    content="打开数据看板页面"
                    placement="top"
                  >
                    <el-button
                  ...
                  <el-tooltip
                    v-if="section.name === 'menuConfigs' && canOpenMenuPage(scope.row)"
                    content="打开菜单页面"
                    placement="top"
                  >
                    <el-button
...
const asRecordList = (value: unknown): VisualizationRecord[] => {
  return Array.isArray(value)
    ? value
        ...
        .map(item => item as VisualizationRecord)
    : [];
};
...
const sections = computed(() => [
  {
    name: 'chartLibrary',
    label: '图表库',
    items: chartLibrary.value,
    keyProp: 'chartType',
    keyLabel: '图表类型',
  },
  {
    name: 'visualizationConfigs',
    label: '可视化配置',
    items: visualizationConfigs.value,
    keyProp: 'configType',
    keyLabel: '配置类型',
  },
  {
...
const canViewConfig = (sectionName: string, record: VisualizationRecord) => {
  ...
  if (sectionName === 'visualizationConfigs')
    return Boolean(record.configType || record.configIndex);
...
const previewButtonTip = (sectionName: string, record: VisualizationRecord) => {
  ...
  return chartOptionFromRecord(record) ? '预览图表快照' : '预览图表（历史记录缺少 ECharts 快照）';
};
...
const handleRecordsUpdated = (detail: DataVisualizationRecordEventDetail) => {
  ...
  sessionRecordId.value = typeof detail.sessionRecordId === 'string' ? detail.sessionRecordId : '';
