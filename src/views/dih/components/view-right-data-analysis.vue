        ...
        <div class="analysis-section">
          ...
          <div v-else class="result-list">
            <div v-for="result in serviceResults" :key="recordId(result)" class="result-item">
              ...
              <pre class="json-result">{{ prettyJson(result.analysisResult || result.result || result) }}</pre>
            </div>
          </div>
        </div>
      </el-tab-pane>
...
const recordId = (record: RowRecord | DataAnalysisRecord) =>
  cellText(record.id || record.recordId || record.serviceTaskId || record.title || record.stage || JSON.stringify(record))
...
const timelineType = (item: RowRecord): TimelineType => {
  ...
  if (['primary', 'success', 'warning', 'danger', 'info'].includes(type)) return type as TimelineType
...
<style scoped>
.panel { display: flex; flex-direction: column; height: 100%; box-sizing: border-box; background: #f5f7fa; color: #303133; overflow: hidden; }
.right-panel { padding: 0; }
.right-tabs { flex: 1; min-height: 0; overflow: hidden; background: #fff; }
:deep(.el-tabs__content) { height: calc(100% - 40px); overflow-y: auto; }
:deep(.el-tabs__nav) { padding: 0 12px; }
:deep(.el-tabs__item) { height: 40px; line-height: 40px; }
.analysis-section { padding: 12px; }
.section-toolbar, .result-header { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 12px; }
.section-title, .timeline-title { color: #303133; font-weight: 600; }
.section-subtitle { margin-top: 4px; color: #909399; font-size: 12px; }
.data-table { width: 100%; }
.result-list { display: flex; flex-direction: column; gap: 12px; }
.result-item { padding: 12px; border: 1px solid #ebeef5; border-radius: 6px; background: #fff; }
.json-result { max-height: 420px; margin: 0; padding: 12px; overflow: auto; border-radius: 6px; background: #f7f8fa; color: #303133; font-size: 12px; line-height: 1.6; white-space: pre-wrap; word-break: break-word; }
.timeline-content { margin-top: 6px; color: #606266; font-size: 13px; line-height: 1.6; white-space: pre-wrap; }
</style>
